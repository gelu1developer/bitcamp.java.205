...
//	데이터를 저장 -> 변수를 이용해서 메모리에 저장
	//직접참조 데이터를 막기위해서 private선언
	private String name; //클래스의 영역에서 정의 -> 인스턴스 변수, 인스턴스가 생성될 때 변수의 메모리공간이 생성된다. 초기화를 생략 가능하다.
...
//		this.grade = grade;
//		this.email = email;
//		this.birthday = birthday;  //요구사항에 맞게 지역변수 삭제
//		this.address = address;
...
//		this.major = major;
	...
	Member(){//따로 생성을 해야 멤버가 없는 처리를 할 수 있다.
		this.phoneNumber = "000-0000-0000";
	} 
//	②위에서 정의한 정보를 출력하는 메소드 정의
//	기능 -> 메소드
	void showInfo() {
		System.out.println("이름 : " + this.name); //인스턴스 자기자신의 참조값을 가지는 참조변수
	...
	public void setName(String name) { //외부에서 접근할 수 있는 규칙을 세워준다.
		this.name = name;
	}

	public String getName() {//데이터를 반환해
		return this.name;
	}
...
//	④main() 메소드에서 두 가지 생성자를 이용해서 인스턴스 생성하고 출력 메소드를 통해 저장된 데이터 출력
	...
	public static void main(String[] args) {
		...
		Member member1 = new Member("철수", "010-1234-5678", "음악", 4, "ch@gmail.com", "2000-10-01", "서울");
		member1.name = "영희";
		member1.showInfo();
